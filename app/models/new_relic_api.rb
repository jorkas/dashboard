...
module NewRelicApi
  ...
  class BaseResource < ActiveResource::Base #:nodoc:
    ...
    class << self
      ...
      def headers
        h = {'x-license-key' => NewRelicApi.license_key || CONFIG['new_relic_key'] || NewRelic::Control.instance['license_key']}
        h['Authorization'] = 'Basic ' + ["#{NewRelicApi.email}:#{NewRelicApi.password}"].pack('m').delete("\r\n") if NewRelicApi.email
        h
      end

      def site_url
        host = NewRelicApi.host || CONFIG['new_relic_host'] || NewRelic::Control.instance.api_server.name
        port = NewRelicApi.port || CONFIG['new_relic_port'] || NewRelic::Control.instance.api_server.port
